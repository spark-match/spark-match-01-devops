...
| [`configure-repo-rulesets.sh`](./configure-repo-rulesets.sh) | Creates the `spark-match-default-branch-protection` ruleset per-repo (GitHub Free has no org-level rulesets). Adds `pull_request` (1 approval + code-owner review), `non_fast_forward`, `required_linear_history`, and optional `required_status_checks`. Bypass actor: `OrganizationAdmin` with `bypass_mode: always`. | `./configure-repo-rulesets.sh [--dry-run] [--repos r1,r2] [--status-checks "Plan (dev),Checkov"] [--approvals N] [--delete-existing]` | No |
| [`apply-strict-rulesets.sh`](./apply-strict-rulesets.sh) | Stricter successor to `configure-repo-rulesets.sh`. Same ruleset but `bypass_mode: pull_request` — admins can merge PRs without meeting review rules but cannot push direct to `main`. Hardcoded for the three primary repos (`spark-match-01-devops`, `spark-match-02-infrastructure`, `spark-match-03-backend`) with per-repo required status checks. | `./apply-strict-rulesets.sh [--dry-run] [--repos r1,r2]` | No |
